...
public:
     ...
     *       caller still owns both credentials and can retry the assignment once the host can
     *       accept the release. Only after a successful (ok/stale) return is other drained.
     */
 ...
 *
 * The writer is the plugin-facing end of icalls::call()/iinvoke::invoke(): the ABI only ever sees
 * the borrowed iwriter*, while the std::string target and this class stay on the producing side
 * and never cross the boundary. A call that fails leaves partial output discarded by the host,
 * so a writer that also refuses to grow past its limit keeps the result all-or-nothing.
 *
