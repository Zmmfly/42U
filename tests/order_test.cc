 ...
 * @param expected_order Expected planned identities when expected is abi::v1::ok.
...
void expect_plan(const char* label, const std::vector<order_node>& nodes,
                 const std::vector<std::string>& initialized, abi::v1::status expected,
                 const std::vector<std::string>& expected_order, const char* needle = nullptr)
{
    ...
    const abi::v1::status actual = u42::plan_order(nodes, initialized, out, error);
    if (actual != expected) {
...
TEST_CASE(priority_ascending)
{
    ...
    expect_plan("priority", nodes, {}, abi::v1::ok, {"high", "mid", "low"});
}
...
TEST_CASE(priority_tie_uses_utf8_byte_order)
{
    ...
    expect_plan("tie-insertion-order", ascending, {}, abi::v1::ok, expected);

    const std::vector<order_node> reversed = {make_node("\xC3\xA9", 3), make_node("z", 3),
                                             make_node("ab", 3), make_node("a", 3),
                                             make_node("A", 3)};
    expect_plan("tie-reversed-input", reversed, {}, abi::v1::ok, expected);
}
...
TEST_CASE(hard_edges_override_priority)
{
    ...
    expect_plan("hard-edges", nodes, {}, abi::v1::ok, {"app", "svc", "log"});

    std::vector<order_node> mirrored;
    mirrored.push_back(make_node("a", 10, {"b"}));
    mirrored.push_back(make_node("b", 0, {}, {"a"}));
    expect_plan("hard-edge-declared-twice", mirrored, {}, abi::v1::ok, {"a", "b"});
}
...
TEST_CASE(duplicate_edges_are_deduplicated)
{
    ...
    expect_plan("duplicate-edges", nodes, {}, abi::v1::ok, {"a", "b"});
}

TEST_CASE(self_edges_are_cycles)
{
    expect_plan("self-before", {make_node("solo", 0, {"solo"})}, {}, abi::v1::cycle, {}, "'solo'");
    expect_plan("self-after", {make_node("solo", 0, {}, {"solo"})}, {}, abi::v1::cycle, {},
                "'solo'");
    const std::vector<order_node> both = {make_node("solo", 0, {"solo"}, {"solo"}),
                                         make_node("other", 1)};
    expect_plan("self-both", both, {}, abi::v1::cycle, {}, "itself");
}
...
TEST_CASE(mutual_edges_are_cycles)
{
    const std::vector<order_node> before_cycle = {make_node("a", 0, {"b"}),
                                                 make_node("b", 1, {"a"})};
    expect_plan("mutual-before", before_cycle, {}, abi::v1::cycle, {}, "cycle");

    const std::vector<order_node> after_cycle = {make_node("a", 0, {}, {"b"}),
                                                make_node("b", 1, {}, {"a"})};
    expect_plan("mutual-after", after_cycle, {}, abi::v1::cycle, {}, "cycle");
}
...
TEST_CASE(graph_cycle_clears_output)
{
    ...
    expect_plan("graph-cycle", nodes, {}, abi::v1::cycle, {}, "cycle");

    const std::vector<order_node> trailing = {make_node("a", 0), make_node("b", 0, {"c"}),
                                             make_node("c", 0, {"b"})};
    expect_plan("graph-cycle-tail", trailing, {}, abi::v1::cycle, {}, "cycle");
}

TEST_CASE(unknown_targets_are_not_found)
{
    expect_plan("unknown-before", {make_node("a", 0, {"ghost"})}, {}, abi::v1::not_found, {},
                "'ghost'");
    expect_plan("unknown-after", {make_node("a", 0, {}, {"ghost"})}, {}, abi::v1::not_found, {},
                "'ghost'");
    const std::vector<order_node> mixed = {make_node("a", 0), make_node("b", 0, {}, {"ghost"})};
    expect_plan("unknown-after-mixed", mixed, {}, abi::v1::not_found, {}, "'ghost'");
    expect_plan("empty-target", {make_node("a", 0, {""})}, {}, abi::v1::not_found, {}, "'a'");
}
...
TEST_CASE(initialized_constraints_are_enforced)
{
    ...
    expect_plan("before-initialized", impossible, {"init"}, abi::v1::invalid_state, {}, "'init'");
    ...
    const std::vector<order_node> satisfied = {make_node("b", 1, {}, {"init"}),
                                               make_node("a", 5)};
    expect_plan("after-initialized", satisfied, {"init"}, abi::v1::ok, {"b", "a"});

    const std::vector<order_node> only_after = {make_node("a", 0, {}, {"init"})};
    expect_plan("after-initialized-only", only_after, {"init"}, abi::v1::ok, {"a"});
}

TEST_CASE(duplicate_identities_are_rejected)
{
    const std::vector<order_node> pending_duplicate = {make_node("dup", 0), make_node("dup", 1)};
    expect_plan("duplicate-pending", pending_duplicate, {}, abi::v1::duplicate, {}, "'dup'");

    const std::vector<order_node> triple = {make_node("dup", 0), make_node("dup", 1),
                                            make_node("dup", 2)};
    expect_plan("duplicate-pending-triple", triple, {}, abi::v1::duplicate, {}, "'dup'");

    const std::vector<order_node> single = {make_node("x", 0)};
    expect_plan("duplicate-initialized", single, {"init", "init"}, abi::v1::duplicate, {},
                "'init'");
    expect_plan("pending-and-initialized", single, {"x"}, abi::v1::duplicate, {}, "'x'");
}

TEST_CASE(empty_identities_are_rejected)
{
    const std::vector<order_node> blank = {make_node("a", 0), make_node("", 1)};
    expect_plan("empty-pending-id", blank, {}, abi::v1::invalid_argument, {}, "empty");

    const std::vector<order_node> valid = {make_node("a", 0)};
    expect_plan("empty-initialized-id", valid, {""}, abi::v1::invalid_argument, {}, "empty");
    expect_plan("empty-pending-first", {make_node("", 0)}, {}, abi::v1::invalid_argument, {},
                "empty");
    ...
    expect_plan("empty-pending-twice", two_blank, {}, abi::v1::invalid_argument, {}, "empty");
    expect_plan("empty-initialized-twice", valid, {"", ""}, abi::v1::invalid_argument, {},
                "empty");
}
...
TEST_CASE(ready_set_is_reselected_after_every_step)
{
    ...
    expect_plan("reselection", nodes, {}, abi::v1::ok, {"c", "d", "b", "a"});
}
...
TEST_CASE(output_and_error_are_hygienic)
{
    ...
    CHECK(u42::plan_order(two, {}, out, error) == abi::v1::ok);
    CHECK(out.size() == 2);
    CHECK(error.empty());

    error = "stale again";
    CHECK(u42::plan_order(one, {}, out, error) == abi::v1::ok);
    CHECK(out.size() == 1);
    CHECK(out[0] == 0);
    CHECK(error.empty());
    ...
    CHECK(u42::plan_order(broken, {}, out, error) == abi::v1::not_found);
    CHECK(out.empty());
    CHECK(!error.empty());
...
TEST_CASE(long_chain_stays_linear)
{
    ...
    expect_plan("long-chain", nodes, {}, abi::v1::ok, expected);
}
