 ...
 */
abi::v1::entry_fn resolve_entry(void* handle, std::string& error)
{
#if defined(_WIN32)
    const FARPROC symbol = ::GetProcAddress(static_cast<HMODULE>(handle), abi::v1::entry_name);
    if (symbol == nullptr) {
        error = "cannot resolve " + std::string(abi::v1::entry_name) + ": " + last_error_message();
        return nullptr;
    }
    return reinterpret_cast<abi::v1::entry_fn>(symbol);
#else
    ::dlerror(); // Clear stale state so the next diagnostic belongs to this lookup.
    void* symbol = ::dlsym(handle, abi::v1::entry_name);
    const char* diagnostic = ::dlerror();
    if (diagnostic != nullptr) {
        error = "cannot resolve " + std::string(abi::v1::entry_name) + ": " + diagnostic;
        return nullptr;
    }
    if (symbol == nullptr) {
        error = std::string(abi::v1::entry_name) + " resolves to null";
        return nullptr;
    }
    return reinterpret_cast<abi::v1::entry_fn>(symbol);
#endif
...
abi::v1::status plug::open(const std::filesystem::path& path, std::string& error)
{
    error.clear();
    if (handle_ != nullptr) {
        error = "library already open: '" + path_.string() + "'";
        return abi::v1::invalid_state;
    }
    if (path.empty()) {
        error = "plugin library path is empty";
        return abi::v1::invalid_argument;
    }
    ...
    if (ec) {
        ...
        return ec == std::errc::no_such_file_or_directory ? abi::v1::not_found : abi::v1::failed;
    }
    ...
    abi::v1::entry_fn entry = resolve_entry(handle, diagnostic);
    if (entry == nullptr) {
        error = diagnostic;
    ...
    abi::v1::iplug_fty* factory = nullptr;
    const abi::v1::status negotiated = entry(abi::v1::abi_major, &factory);
    if (negotiated != abi::v1::ok) {
        error = "ABI negotiation failed for '" + canonical.string() + "': requested major " +
                std::to_string(abi::v1::abi_major) + ", library returned status " +
