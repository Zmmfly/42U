...
/**
 * @brief Deliver one capability notice to its watch owner.
 *
...
void deliver_notice(engine& runtime, cap_notice& notice) noexcept
{
    ...
    // Both views borrow the notice and this array; they expire when the callback returns.
    a::caps_desc caps{};
    caps.struct_size = sizeof(a::caps_desc);
    caps.interface_count = static_cast<std::uint32_t>(notice.capabilities.interfaces.size());
    caps.interfaces =
        notice.capabilities.interfaces.empty() ? nullptr : notice.capabilities.interfaces.data();
